...
class SeleniumCookie(object):
    ...
    def chromeLogin(self):
        try:
            ...
            password = self.wait.until(
                EC.presence_of_element_located((By.XPATH, '//*[@id="pl_login_form"]/div/div[3]/div[2]/div/input')))
            password.click()
    ...
    def cycleScroll(self):
        try:
            ...
            while (True):
                ...
                time.sleep(10)
                photolist = self.wait.until(
                    EC.presence_of_all_elements_located((By.XPATH, "//ul[@class='photo_album_list clearfix']")))
    ...
    def getSrcaddrByType(self, srctype):
        try:
            if (srctype == '.gif'):
                srcitem = self.wait.until(EC.visibility_of_element_located((By.XPATH,
                                                                            "//div[contains(@class,'W_layer layer_multipic_preview')]//div[@node-type='gif_box']//video")))
                return srcitem.get_attribute('src')
            else:
                srcitem = self.wait.until(EC.visibility_of_element_located(
                    (By.XPATH, "//div[contains(@class,'W_layer layer_multipic_preview')]//img[@hidefocus]")))
                return srcitem.get_attribute('src')
        except:
            return None
    ...
    def closePhoto(self):
        try:
            closebtn = self.wait.until(EC.element_to_be_clickable(
                (By.XPATH, "//div[contains(@class,'W_layer layer_multipic_preview')]//*[@title='关闭']")))
            closebtn.click()
            time.sleep(2)
        except NoSuchElementException:
